import servicesFurnitureImage from "../images/services/services-furniture.png";
import servicesSillImage from "../images/services/services-sill.png";
import servicesInteriorImage from "../images/services/services-interior.png";
import icon_1 from "../images/icons/advantages-icon-1.svg";
import icon_2 from "../images/icons/advantages-icon-2.svg";
import icon_3 from "../images/icons/advantages-icon-3.svg";
import icon_4 from "../images/icons/advantages-icon-4.svg";
import icon_5 from "../images/icons/advantages-icon-5.svg";
import icon_6 from "../images/icons/advantages-icon-6.svg";
import icon_7 from "../images/icons/advantages-icon-7.svg";
import icon_8 from "../images/icons/advantages-icon-8.svg";
import phaseIcon_1 from "../images/phases-images/1.svg";
import phaseIcon_2 from "../images/phases-images/2.svg";
import phaseIcon_3 from "../images/phases-images/3.svg";
import phaseIcon_4 from "../images/phases-images/4.svg";
import material_wood_1 from "../images/surfaces/wood/1.jpg";
import material_wood_2 from "../images/surfaces/wood/2.jpg";
import material_wood_3 from "../images/surfaces/wood/3.jpg";
import material_wood_4 from "../images/surfaces/wood/4.jpg";
import material_wood_5 from "../images/surfaces/wood/5.jpg";
import material_stone_1 from "../images/surfaces/stone/1.jpg";
import material_stone_2 from "../images/surfaces/stone/2.jpg";
import material_stone_3 from "../images/surfaces/stone/3.jpg";
import material_stone_4 from "../images/surfaces/stone/4.jpg";
import material_stone_5 from "../images/surfaces/stone/5.jpg";
import material_stone_6 from "../images/surfaces/stone/6.jpg";
import material_stone_7 from "../images/surfaces/stone/7.jpg";
import material_stone_8 from "../images/surfaces/stone/8.jpg";
import material_stone_9 from "../images/surfaces/stone/9.jpg";
import material_stone_10 from "../images/surfaces/stone/10.jpg";
import material_stone_11 from "../images/surfaces/stone/11.jpg";
import material_stone_12 from "../images/surfaces/stone/12.jpg";
import material_stone_13 from "../images/surfaces/stone/13.jpg";
import material_stone_14 from "../images/surfaces/stone/14.jpg";
import material_stone_15 from "../images/surfaces/stone/15.jpg";
import material_stone_16 from "../images/surfaces/stone/16.jpg";
import material_stone_17 from "../images/surfaces/stone/17.jpg";
import material_cement_1 from "../images/surfaces/cement/1.jpg";
import material_cement_2 from "../images/surfaces/cement/2.jpg";
import material_cement_3 from "../images/surfaces/cement/3.jpg";
import material_cement_4 from "../images/surfaces/cement/4.jpg";
import material_cement_5 from "../images/surfaces/cement/5.jpg";
import material_cement_6 from "../images/surfaces/cement/6.jpg";
import material_cement_7 from "../images/surfaces/cement/7.jpg";
import material_design_1 from "../images/surfaces/design/1.jpg";
import material_design_2 from "../images/surfaces/design/2.jpg";
import material_design_3 from "../images/surfaces/design/3.jpg";
import material_design_4 from "../images/surfaces/design/4.jpg";
import material_design_5 from "../images/surfaces/design/5.jpg";
import material_design_6 from "../images/surfaces/design/6.jpg";
import material_design_7 from "../images/surfaces/design/7.jpg";
import material_design_8 from "../images/surfaces/design/8.jpg";
import material_design_9 from "../images/surfaces/design/9.jpg";
import material_design_10 from "../images/surfaces/design/10.jpg";
import material_design_11 from "../images/surfaces/design/11.jpg";
import material_design_12 from "../images/surfaces/design/12.jpg";
import advice_1 from "../images/advices/advice1.jpg";
import advice_2 from "../images/advices/advice2.jpg";
import advice_3 from "../images/advices/advice3.jpg";
import portfolio_1 from "../images/portfolio/portfolio_1.jpg";
import portfolio_2 from "../images/portfolio/portfolio_2.jpg";
import portfolio_3 from "../images/portfolio/portfolio_3.jpg";
import supplier_LAMINAM from "../images/suppliers/LAMINAM.png";
import supplier_thesize from "../images/suppliers/thesize.png";
import supplier_cottodeste from "../images/suppliers/cottodeste.png";
import supplier_ariostea from "../images/suppliers/ariostea.png";
import supplier_panaria from "../images/suppliers/panaria.png";
import supplier_florim from "../images/suppliers/Florim.png";
import supplier_Ariostea_ULTRA from "../images/suppliers/Ariostea_ULTRA.png";
import supplier_neolith from "../images/suppliers/neolith.png";
import supplier_kerlite from "../images/suppliers/kerlite.png";
import supplier_magnum from "../images/suppliers/magnum.png";
import leadBgImage from "../images/lead-bg-image.jpg";
import contactsEntranceImage from "../images/contactsEntranceImage.jpg";
import postFormOffer from "../images/applicability-table.png";
import applicabilityTable from "../images/applicability-table.png";

export const textContent = {
  postForm: {
    heading: "Оставить заявку",
    subHeading: "Подберем оптимальное решение для вашего интерьера",
  },
  pricing: {
    heading: "расчет цены на изделия и услуги",
    textMajor:
      "В связи с минимальным размером листа выбранного вами материала \n(как правило, это 3000 х 1000 мм, 3000 х 1500 мм), и особенностью его \nраскроя конечная цена не поддается расчету, применяемому в отношении \nтрадиционных облицовочных материалов.",
    textMinor:
      "У нас часто проводятся акции на одиночные изделия из материалов, \nкоторыми мы располагаем в остатках.",
    buttonText: "заказать расчет",
  },
  contacts: {
    heading: "Наши контакты",
    address: "г. Ивантеевка, ул. Толмачева 1/2",
    phonePrimary: "+7 (915) 047 48 11",
    phoneAdditional: "+7 (963) 782 23 47",
    emailAddress: "fioredipietra@yandex.ru",
    howToGetText: "К нам можно добраться любым видом наземного транспорта:",
    byBusText:
      "От м. ВДНХ на маршруте 316 до остановки ул. Новая Слобода. Далее пересечь двор «П-образного» жилого дома до дальней от остановки секции, вход на углу дома.",
    byTrainText:
      "Москва Ярославская – Фрязино Пассажирская до остановки «Ивантеевка 2», далее пешком на восток через жилой массив к зданию с зелеными башенками на крыше, крыльцо с южного торца здания.",
    byVehicleText:
      "От Ярославского шоссе через ближайшие въезды в город Ивантеевку до ул. Карла Маркса. В навигаторе задайте «Студия Каменный цветок» или ул. «Карла Маркса, дом 2»",
    landmarksDescription:
      "Общие визуальные ориентиры: дом с бежевым фасадом и зелеными башенками на крыше, на кованых решетках входных дверей и окнах студии увидите наш логотип, тротуар тоже заметный, выложен цветной брусчаткой.",
  },
  lead: {
    leadTitle: "Студия Керамогранита «Каменный цветок»",
    leadText1: "Производство изделий из листового керамического проката",
    leadText2: "Дизайн интерьеров",
    leadText3: "Cветотехнические решения для дома",
  },
  advantages: {
    title: "Преимущества материала",
    shortTextBeforeAccent: "Мы работаем с ",
    shortTextAccent: "листовым керамическим прокатом",
    shortTextAfterAccent:
      " от ведущих производителей: Laminam, Cotto d’Este, Panaria, Florim, Thesize. Это керамогранит толщиной от 3 до 6 мм и размером листа от 3 до 4,5 кв. метров с уникальными механическими характеристиками и эстетикой. Применяется в качестве финишного покрытия в интерьерах, фасадах, при производстве мебели.",
    linkText: "Принципиальное отличие от традиционного керамогранита",
    expandedText:
      "Применяется технология горизонтального формования сырой керамической массы при формовании плиты и дальнейший прокат (давление 15 тонн на кв. метр), с последующим  обжигом и сушкой (традиционный керамогранит и керамическая плитка формуются на вертикальных прессах с максимальным давлением 8 тонн). В результате в структуре материала после обжига и сушки не возникает пустот и, как следствие, остаточных напряжений (присущих традиционной технологии прессования в металлическую рамку), что улучшает прочностные и эксплуатационные характеристики, обеспечивает все свойства традиционного керамогранита при существенно меньшей толщине и многократно большей площади плиты. Появляются такие механические свойства, как гибкость и упругость в небольших амплитудах, не присущие другим керамическим материалам. Прокат толщиной 3 мм можно сгибать с радиусом 6 метров (например, оклеивание сводов тоннелей одним листом).",
  },
  disadvantages: {
    title: "Недостатки популярных аналогов в сегменте столешниц и фасадов",
    disadvantages1:
      "Натуральный камень не обладает достаточной химической стойкостью: мрамор легко испортить вином или напитками, гранит облит оптическим эпоксидным составом, который царапается и мутнеет от кислотосодержащих моющих средств",
    disadvantages2:
      "Большой удельный вес слэбов и плитки из натурального камня на фасадах зданий существенно увеличивают нагрузку на фундамент и при клеевом способе  крепления на фасаде не всегда выдерживают несколько погодных циклов",
    disadvantages3:
      "Кварцевый агломерат не защищён от ультрафиолета, поэтому светлые его виды могут пожелтеть на солнце",
  },
  phases: {
    title: "Этапы работы",
    phases1: "Обсуждение проекта \nи расчет стоимости",
    phases2: "Выезд замерщика \nна объект",
    phases3: "Оформление \nдоговора",
    phases4: "Производство \nи монтаж изделия",
  },
  suppliers: {
    heading: "Поставщики",
    subheadingMaterials: "Мы работаем с лучшими материалами:",
    subheadingSuppliers:
      "От крупнейших производителей листового керамического проката:",
  },
  surfaces: {
    heading: "Варианты поверхностей",
    shortText:
      "Помимо изображенных на поверхности материала природных и художественных \nликов, разработанных \nдизайнерами Италии и Испании, материал выполнен \nс разнообразными рельефами: полированный,шелковистый, матовый, \nповторяющий грубую обработанную поверхность дерева и камня.",
    expandedText:
      "Появились на рынке образцы, пропускающие свет, как фарфор. \nНа вашем экране сложно увидеть реальную \nцветовую палитру, а рельеф \nи подавно не передается картинкой.  В нашем шоу-руме и при посещении \nобъекта замерщиком с образцами материала вы оцените не только расцветку, \nно и поведение материала в месте его установки: блики, тактильность, \nсочетание с мебелью, освещением и другими компонентами вашего интерьера.",
  },
  portfolio: {
    heading: "Портфолио",
  },
};

export const servicesItems = [
  {
    image: servicesFurnitureImage,
    heading: "Производство керамических столешниц, мебели",
    description:
      "Кухонные столешницы и фартуки, мебель для ванной, подоконники. Столовая мебель с деревянными подстольями в классическом стиле",
    _id: "1",
  },
  {
    image: servicesSillImage,
    heading: "Реновация подоконников",
    description:
      "Экологичное использование старых подоконников: без демонтажа, без повреждения откосов и стен, облицовка керамическими «винирами» за один день",
    _id: "2",
  },
  {
    image: servicesInteriorImage,
    heading: "Отделка интерьеров. Фасадная и интерьерная подсветка",
    description:
      "Полы, стены и потолки из керамики. Контурная подсветка и встроенные светильники.",
    _id: "3",
  },
];

export const advantagesIconsList = {
  icon_1: icon_1,
  icon_2: icon_2,
  icon_3: icon_3,
  icon_4: icon_4,
  icon_5: icon_5,
  icon_6: icon_6,
  icon_7: icon_7,
  icon_8: icon_8,
};

export const phasesIcons = {
  firstIcon: phaseIcon_1,
  secondIcon: phaseIcon_2,
  thirdIcon: phaseIcon_3,
  fourthIcon: phaseIcon_4,
};

export const surfacesItems = [
  {
    _id: "602e70f5552c6a58c4d46fae",
    title: "Дерево",
    examples: [
      {
        _id: "602eb8c3faaf9e5504f2148b",
        description: "Название материала 1",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_wood_1,
      },
      {
        _id: "602eb8ccfaaf9e5504f2148c",
        description: "Название материала 2",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_wood_2,
      },
      {
        _id: "602eb8d4faaf9e5504f2148d",
        description: "Название материала 3",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_wood_3,
      },
      {
        _id: "602eb8dcfaaf9e5504f2148e",
        description: "Название материала 4",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_wood_4,
      },
      {
        _id: "602eb8dcfaaf9e5584f2148e",
        description: "Название материала 5",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_wood_5,
      },
    ],
  },
  {
    _id: "602e796c71bf9e4ba43d4a00",
    title: "Камень",
    examples: [
      {
        _id: "602ed511e4754253b804d197",
        description: "Название материала 1",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_1,
      },
      {
        _id: "602ed518e4754253b804d198",
        description: "Название материала 2",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_2,
      },
      {
        _id: "602ed520e4754253b804d199",
        description: "Название материала 3",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_3,
      },
      {
        _id: "602ed526e4754253b804d19a",
        description: "Название материала 4",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_4,
      },
      {
        _id: "602ed531e4754253b804d19b",
        description: "Название материала 5",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_5,
      },
      {
        _id: "602ed537e4754253b804d19c",
        description: "Название материала 6",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_6,
      },
      {
        _id: "602ed53ee4754253b804d19d",
        description: "Название материала 7",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_7,
      },
      {
        _id: "602ed544e4754753b804d19e",
        description: "Название материала 8",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_8,
      },
      {
        _id: "602ed544e47542538804d19e",
        description: "Название материала 9",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_9,
      },
      {
        _id: "602ed544e4754253b894d19e",
        description: "Название материала 10",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_10,
      },
      {
        _id: "602ed544e5754253b804d19e",
        description: "Название материала 11",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_11,
      },
      {
        _id: "602e3544e4754253b804d19e",
        description: "Название материала 12",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_12,
      },
      {
        _id: "602ed544e4754203b804d19e",
        description: "Название материала 13",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_13,
      },
      {
        _id: "602ed544e0754253b804d19e",
        description: "Название материала 14",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_14,
      },
      {
        _id: "602ed544e4759253b804d19e",
        description: "Название материала 15",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_15,
      },
      {
        _id: "602ed544e4754253b704d19e",
        description: "Название материала 16",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_16,
      },
      {
        _id: "602ed544e4754253b804d198",
        description: "Название материала 17",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_stone_17,
      },
    ],
  },
  {
    _id: "602e797071bf9e4ba43d4a02",
    title: "Цемент",
    examples: [
      {
        _id: "602ed566e4754253b804d19f",
        description: "Название материала 1",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_cement_1,
      },
      {
        _id: "602ed56ce4754253b804d1a0",
        description: "Название материала 2",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_cement_2,
      },
      {
        _id: "602ed571e4754253b804d1a1",
        description: "Название материала 3",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_cement_3,
      },
      {
        _id: "602ed571e4754253b804d181",
        description: "Название материала 4",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_cement_4,
      },
      {
        _id: "602ed571e4754253b804d191",
        description: "Название материала 5",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_cement_5,
      },
      {
        _id: "602ed571e4754253b804d6a1",
        description: "Название материала 6",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_cement_6,
      },
      {
        _id: "602ed571e4754253b804d101",
        description: "Название материала 7",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_cement_7,
      },
    ],
  },
  {
    _id: "602e797371bf9e4ba43d4a04",
    title: "Дизайн",
    examples: [
      {
        _id: "602ed58fe4754253b804d1a2",
        description: "Название материала 1",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_1,
      },
      {
        _id: "602ed596e4754253b804d1a3",
        description: "Название материала 2",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_2,
      },
      {
        _id: "602ed59be4754253b804d1a4",
        description: "Название материала 3",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_3,
      },
      {
        _id: "602ed59be4754253b904d1a4",
        description: "Название материала 4",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_4,
      },
      {
        _id: "602ed59be4794253b804d1a4",
        description: "Название материала 5",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_5,
      },
      {
        _id: "602ed59be47542538804d1a4",
        description: "Название материала 6",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_6,
      },
      {
        _id: "602ed59be4764253b804d1a4",
        description: "Название материала 7",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_7,
      },
      {
        _id: "602ed59be4750253b804d1a4",
        description: "Название материала 8",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_8,
      },
      {
        _id: "602ed59be4754053b804d1a4",
        description: "Название материала 9",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_9,
      },
      {
        _id: "6020d59be4754253b804d1a4",
        description: "Название материала 10",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_10,
      },
      {
        _id: "802ed59be4754253b804d1a4",
        description: "Название материала 11",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_11,
      },
      {
        _id: "602ed59b94754253b804d1a4",
        description: "Название материала 12",
        manufacturer: "COTTO d'ESTE",
        origin: "Италия",
        style: "Forest",
        surface: "Натуральная матовая",
        image: material_design_12,
      },
    ],
  },
];

export const advicesItems = [
  {
    _id: "600627db7f615e0906fc1822",
    heading: "Межплиточные швы и керамический прокат",
    shortText:
      "Появление листового керамического проката и его использование и применение в отделке привело некоторых заказчиков к желанию достичь абсолюта: “- Раз площадь керамического листа огромная, давайте ещё уберем полностью швы, чтобы была визуальная чистота и сплошной узор по всей поверхности (пола/стены/ потолка)”. С другой стороны, их можно понять: все намучились с испокон веков с межплиточными швами на полах, кухонных фартуках и стенах ванной, где происходят некрасивые и негигиеничные процессы, требующие от домохозяек трудоемкого и неэффективного клининга. В-третьих, затирка швов между листами керамики не всех устраивают по своей эстетике - они выглядят шире, чем у обычной плитки: не такие вогнутые.",
    expandedText:
      "Традиционная плитка конструктивно выполняется с радиусным закруглением кромок, которые визуально скрывают реальную ширину шва. Затирание швов происходит путем предварительной набивки затирочной массы и последующим вымыванием верхнего слоя с формовкой ложбинки.  Смотрится эстетично. Лист керамического проката, как и традиционная плитка, имеет тот же химический состав и так же подвержена линейным расширениям и сжатиям при изменениях температуры. Только в связи с другим линейным размером, она расширяется с большими амплитудами. Поэтому неизбежно наличие швов, если вы не хотите разлома материала, уложенного встык подобно обоям. Раз швы неизбежны, легко превратить их в эффективную деталь декора. В нашем портфолио вы найдете примеры не только затирки швов, но и применение различных профилей из разнообразных материалов, включая декор с применением природного камня, стеклянных и керамических декоративных элементов, применяемых обычно в сочетании с ламинатом, деревом и традиционной плиткой. Не бойтесь неожиданных сочетаний, нарушайте консервативные традиции и создавайте собственный дизайн.",
    image: advice_1,
  },
  {
    _id: "600627e97f615e0906fc1823",
    heading: "Геометрия пространства",
    shortText:
      "Используя в дизайне помещения керамику с размерами листа 3... 5 кв. метра, вы свободны в выборе формата.  Традиции применения керамики и других отделочных покрытий в отделке интерьера и фасадов зданий предполагают, что стандартный способ размещения фрагментов покрытия предполагает наличие либо стыков, либо швов, которые присутствуют как данность. Керамические полы могут быть либо в клетку, либо в прямоугольник.   Деревянное покрытие (массив или имитирующий его ламинат или линолеум) должно располагаться встык с направлением волокон на досках вдоль направления движения по полам.",
    expandedText:
      "Применение керамического проката позволяет корректировать визуальное восприятие помещений за счет большей вариабельности. Вы легко сделаете пространство шире или уже, если на полу формат уложенного листа будет выбран для достижения данного результата.  Узкое помещение, пол которого покрыт досками или ламинатом с продольным рисунком под дерево, всегда выглядит уже, чем на самом деле. Листовая керамика позволяет использовать другие пропорции с сохранением правильной ориентации рисунка или древесных волокон (если поверхность повторяет древесину). Формат листа в нашем шоуруме 100 на 60 см, поверхность Rovere oliato (промасленный дуб), направление рельефа волокон по короткой ширине листа. И ходить удобно, и помещение с размером пола 7 на 3,5 метра выглядит комфортным, почти квадратным. Тонкая нить межлистовых швов (а не рисунок волокон) переключает подсознание “на себя”, воспринимается масштабно-координатной сеткой, корректирующей геометрию в нужную нам сторону.  В общем, на всякий прикус найдутся свои брекеты. Понятно, что в узком тесном помещении (например, как совмещенный санузел в нашем шоу-руме) будет комфортнее находиться, когда на стене лист керамики с горизонтальным расположением рисунка либо поверхность с расцверкой “оникс”, напоминающее облачное небо и расширяющее пространство опять же в подсознании.  А если этот лист на стыке обеспечивает непрерывность рисунка, стена вообще “исчезает”. В портфолио посмотрите комнату со стенами, облицованными листами 300 на 100 см I Naturalli Onice двух оттенков, в котором горизонтальные швы закрыты алюминиевым профилем (грибок) под матовое серебро.  В комнате размером 2,2 на 3 метра ощущается простор открытой смотровой площадки в горах в окружении грозовых туч. Даже пахнет озоном! Кстати, хорошая иллюстрация применения широких (4 см) расшивок между листами керамики: за счет пространства, как бы закрытого профилем, подсознание пытается достроить узор между разными листами, которые никак не стыкуются по рисунку. Раз швы неизбежны, легко превратить их в эффективную деталь декора. В нашем портфолио вы найдете примеры не только затирки швов, но и применение различных профилей из разнообразных материалов, включая декор с применением природного камня, стеклянных и керамических декоративных элементов, применяемых обычно в сочетании с ламинатом, деревом и традиционной плиткой. Не бойтесь неожиданных сочетаний, нарушайте консервативные традиции и создавайте собственный дизайн.",
    image: advice_2,
  },
  {
    _id: "602d158c532af6e64f005751",
    heading: "Глянец или шелк?",
    shortText:
      "Следующая рекомендация относится к применению полированных и матовых поверхностей керамического проката. Со слов дистрибьюторов керамического проката, популярность полированных поверхностей в нашей стране значительно превышает спрос на неё в Европе. Конечно, это незаметно по интерьерам, представленным на сайтах производителей проката, для демонстрации преимуществ материала глянец намного зрелищнее. Но в быту важно не оказаться заложником красоты. При выборе поверхности для стен в помещениях небольшого размера (кухни и санузлы в большинстве наших квартир отличаются размерами от европейских, по крайней мере тех, которые показаны на сайтах) важно учесть, какое освещение будет применяться и не будет ли свет от него “давить” на глаза. Дело еще в том, что полированная поверхность керамического проката имеет совсем другие потребительские свойства, чем полированный натуральный камень, традиционная плитка, акрил и дерево. Полируется глазурь на прокате после обжига. Состав глазури -кварцевое стекло, в отличие от эпоксидной смолы, применяемой для глянцевого покрытия   кварцевого агломерата или природного камня. Толщина прозрачного слоя меньше, чем в зеркалах, но выше, чем перечисленных выше материалов.  Поэтому важно убедиться, что в глаза не попадет прямой свет от светильников или световой поток от них не будет раздражать. Почти все наименования керамического проката выпускаются в нескольких вариантах поверхностей с одинаковой расцветкой. Можно при реализации проекта подобрать либо в одной цветовой гамме по разному бликующие поверхности полированная, полуматовая (шёлк), либо натуральная (глянец не полированный, а в виде шагрени после термической обработки листа).",
    expandedText:
      "Единственное место, где полированная поверхность уместна и может применяться без ограничений, это окна: откосы и подоконники из керамического проката удобны в эксплуатации, повышают освещенность помещения природным светом (наиболее полезным для глаза и настроения) и обладают свойством оптической адаптации (хамелеоны), так как мягко отражают постоянно меняющееся небо и окружающее пространство. Пример использования сочетания разноплановых поверхностей на полу - коридор в нашем шоу-руме: полированная поверхность I naturalli Bianco размещается примыкающей к стенам, поэтому блики от мощных светильников на потолке не слепят. И сочетание с поверхностью Rovere Oliato, глубоко матовым и рельефом, повторяющим брашированную древесину, оптимально сочетает безопасность и эстетику - пол не скользкий и за счёт отражения светового потока выполняет функцию пассивной экологичной подсветки.",
    image: advice_3,
  },
];

export const suppliersItems = [
  {
    _id: "604e60272aa5d7e33be65448",
    link: "https://www.laminam.com/en/",
    isMaterial: false,
    image: supplier_LAMINAM,
  },
  {
    _id: "604e60402aa5d7e33be65449",
    link: "https://www.thesize.es/en/",
    isMaterial: false,
    image: supplier_thesize,
  },
  {
    _id: "604e60492aa5d7e33be6544a",
    link: "https://www.thesize.es/en/",
    isMaterial: true,
    image: supplier_neolith,
  },
  {
    _id: "604e605a2aa5d7e33be6544b",
    link: "https://www.cottodeste.it/",
    isMaterial: true,
    image: supplier_kerlite,
  },
  {
    _id: "604e60652aa5d7e33be6544c",
    link: "https://www.cottodeste.it/",
    isMaterial: false,
    image: supplier_cottodeste,
  },
  {
    _id: "604e60712aa5d7e33be6544d",
    link: "https://www.ariostea.it/",
    isMaterial: false,
    image: supplier_ariostea,
  },
  {
    _id: "604e607d2aa5d7e33be6544e",
    link: "https://www.ariostea.it/",
    isMaterial: true,
    image: supplier_Ariostea_ULTRA,
  },
  {
    _id: "604e60862aa5d7e33be6544f",
    link: "https://www.florim.com/",
    isMaterial: true,
    image: supplier_magnum,
  },
  {
    _id: "604e608e2aa5d7e33be65450",
    link: "https://www.florim.com/",
    isMaterial: false,
    image: supplier_florim,
  },
  {
    _id: "604e609a2aa5d7e33be65451",
    link: "https://www.panaria.it/collezione/zero3-eternity",
    isMaterial: false,
    image: supplier_panaria,
  },
];

export const adminItems = [
  { id: "requests", content: "Заявки" },
  { id: "lead", content: "Главная" },
  { id: "services", content: "Услуги" },
  { id: "advantages", content: "Преимущества" },
  { id: "disadvantages", content: "Недостатки" },
  { id: "phases", content: "Этапы работы" },
  { id: "pricing", content: "Расчёт цен" },
  { id: "surfaces", content: "Варианты поверхностей" },
  { id: "advices", content: "Советы дизайнера" },
  { id: "portfolio", content: "Портфолио" },
  { id: "suppliers", content: "Производители" },
  { id: "postform", content: "Оставить заявку" },
  { id: "contacts", content: "Контакты" },
];

export const portfolioItems = [
  {
    _id: "6040470980bc908d15c8b1cd",
    title: "Стол обеденный",
    category: "Мебель",
    text:
      'Стол обеденный 140х90х75 см (под заказ размер до 310х110 см): \n– Поверхность Oxide Moro 3,5 mm (Laminam,Италия), \n– Дизайн "Каменный цветок" \n– Подстолье массив ясеня, инкрустация шпоном ценных пород (Woodstock, г. Москва) \n– Окраска натуральными маслами с твердым воском (Osmo, Германия) \n– Столярные работы С. Калинин (г. Пушкино) \n– Стоимость от 120 000 руб. (в зависимости от размера столешницы, сорта древесины) \n– Фотоснимок В. Перегуда (г. Ивантеевка)',
    photos: [
      {
        _id: "6040478680bc908d15c8b1d7",
        description: "primer 1",
        image: portfolio_1,
      },
      {
        _id: "6040479080bc908d15c8b1d8",
        description: "primer 2",
        image: portfolio_2,
      },
      {
        _id: "6040479680bc908d15c8b1d9",
        description: "primer 3",
        image: portfolio_3,
      },
    ],
  },
  {
    _id: "6040471280bc908d15c8b1cf",
    title: "Стол обеденный 2",
    category: "Мебель",
    text:
      'Стол обеденный 140х90х75 см (под заказ размер до 310х110 см): \n– Поверхность Oxide Moro 3,5 mm (Laminam,Италия), \n– Дизайн "Каменный цветок" \n– Подстолье массив ясеня, инкрустация шпоном ценных пород (Woodstock, г. Москва) \n– Окраска натуральными маслами с твердым воском (Osmo, Германия) \n– Столярные работы С. Калинин (г. Пушкино) \n– Стоимость от 120 000 руб. (в зависимости от размера столешницы, сорта древесины) \n– Фотоснимок В. Перегуда (г. Ивантеевка)',
    photos: [
      {
        _id: "6040478680bc908d15c8b1d7",
        description: "primer 1",
        image: portfolio_1,
      },
      {
        _id: "6040479080bc908d15c8b1d8",
        description: "primer 2",
        image: portfolio_2,
      },
      {
        _id: "6040479680bc908d15c8b1d9",
        description: "primer 3",
        image: portfolio_3,
      },
    ],
  },
  {
    _id: "6040471780bc908d15c8b1d1",
    title: "Стол обеденный 3",
    category: "Мебель",
    text:
      'Стол обеденный 140х90х75 см (под заказ размер до 310х110 см): \n– Поверхность Oxide Moro 3,5 mm (Laminam,Италия), \n– Дизайн "Каменный цветок" \n– Подстолье массив ясеня, инкрустация шпоном ценных пород (Woodstock, г. Москва) \n– Окраска натуральными маслами с твердым воском (Osmo, Германия) \n– Столярные работы С. Калинин (г. Пушкино) \n– Стоимость от 120 000 руб. (в зависимости от размера столешницы, сорта древесины) \n– Фотоснимок В. Перегуда (г. Ивантеевка)',
    photos: [
      {
        _id: "6040478680bc908d15c8b1d7",
        description: "primer 1",
        image: portfolio_1,
      },
      {
        _id: "6040479080bc908d15c8b1d8",
        description: "primer 2",
        image: portfolio_2,
      },
      {
        _id: "6040479680bc908d15c8b1d9",
        description: "primer 3",
        image: portfolio_3,
      },
    ],
  },
];

export const imagesItems = {
  leadBgImage: leadBgImage,
  contactsEntranceImage: contactsEntranceImage,
  postFormOffer: postFormOffer,
  applicabilityTable: applicabilityTable,
};
